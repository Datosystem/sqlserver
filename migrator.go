...
func (m Migrator) AlterColumn(value interface{}, field string) error {
	return m.RunWithValue(value, func(stmt *gorm.Statement) error {
		if field := stmt.Schema.LookUpField(field); field != nil {
			if field.Unique || field.TagSettings["INDEX"] != "" {
				return nil
			}
			// log.Println("Migrating ", stmt.Table+"."+field.DBName)
			...
			//CUSTOM NOSTRO PER FAR FUNZIONARE MSSQL
			defaultVal := field.DefaultValue
			statements := []string{`
				DECLARE @ConstraintName nvarchar(200);
				SELECT @ConstraintName = Name FROM SYS.DEFAULT_CONSTRAINTS
				WHERE PARENT_OBJECT_ID = OBJECT_ID('@table') AND PARENT_COLUMN_ID = (SELECT column_id FROM sys.columns WHERE NAME = N'@column' AND object_id = OBJECT_ID(N'@table'));
				IF @ConstraintName IS NOT NULL
					EXEC('ALTER TABLE @table DROP CONSTRAINT ' + @ConstraintName);
				SET @ConstraintName = NULL;
				ALTER TABLE @table ALTER COLUMN @column @isNull;`}
			args := []any{sql.Named("table", clause.Table{Name: stmt.Table, Raw: true}), sql.Named("column", clause.Column{Name: field.DBName, Raw: true}), sql.Named("isNull", fieldType)}
			if defaultVal != "" {
				if strings.Contains(defaultVal, " ") || field.GORMDataType == "string" {
					defaultVal = "'" + defaultVal + "'"
				}
				statements = append(statements, `ALTER TABLE @table ADD CONSTRAINT `+stmt.Table+`_`+field.DBName+` DEFAULT @default FOR @column;`)
				args = append(args, sql.Named("default", clause.Expr{SQL: defaultVal}))
			}
			return m.DB.Exec(strings.Join(statements, "\n"), args...).Error
		}
		/*
				DA SQL AGGIORNATO
			if stmt.Schema != nil {
